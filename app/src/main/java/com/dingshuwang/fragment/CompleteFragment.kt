 ...
 */
class CompleteFragment : BaseFragment(), DataView {
    ...
    private fun doData() {
        val url = String.format(APIURL.complete_url, Constants.USER_ID)
    ...
    override fun onGetDataSuccess(result: String, requestTag: String) {
        if (null != result) {
            ...
            if (null != item && null != item.shops) {
                listView!!.adapter = TopayAdapter(item.shops!!, mActivity, 4)
            }
        }
    }
