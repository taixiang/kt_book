...
class ExpOrderFragment : BaseFragment(), DataView {
    ...
    private fun doData() {
        val url = String.format(APIURL.WAIT_ORDER, Constants.USER_ID)
    ...
    override fun onGetDataSuccess(result: String, requestTag: String) {
        if (null != result) {
            ...
            if (null != item && null != item.shops) {
                listView!!.adapter = TopayAdapter(item.shops!!, mActivity, 2)
            }
        }
    }
    ...
    companion object {
        ...
        fun newInstance(): ExpOrderFragment {
            return ExpOrderFragment()
        }
    }
}
